...
const ONB = {
  ...
  async connectAndEnter(){
    ...
    try {
      ...
      // Restore session backup if vault had no rewards
      if(S.noet===0){
        const bk=sessionStorage.getItem(`noetica_noet_${addr}`)
        if(bk && parseInt(bk)>0){
          S.noet=parseInt(bk)||0
          S.totalEarned=parseInt(sessionStorage.getItem(`noetica_earned_${addr}`))||0
        }
      }
      if(!S.name) S.name=''
...
const APP = {
  init(){
    // Restore from sessionStorage backup if vault failed to load rewards
    if(S.address && S.noet===0){
      const bk=sessionStorage.getItem(`noetica_noet_${S.address}`)
      const bkE=sessionStorage.getItem(`noetica_earned_${S.address}`)
      if(bk && parseInt(bk)>0){
        S.noet=parseInt(bk)||0
        S.totalEarned=parseInt(bkE)||0
        console.log('[NOETICA] Restored',S.noet,'NOET from session backup')
      }
    }
